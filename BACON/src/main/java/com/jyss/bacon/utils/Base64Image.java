...
public class Base64Image {
	...
	public static boolean GenerateImage(String imgStr, String filePath) { // 对字节数组字符串进行Base64解码并生成图片
		...
		imgStr = imgStr.replace("<img alt=\"\" src=\"data:image/jpeg;base64,",
				"");// 这部分会出现乱码截取掉
		imgStr = imgStr.replace(" ", "+");// 可能会出现乱码的问题
		...
		try {
			...
			// 生成图片\content\images\tempimage
			// HttpServletRequest request =
			// ServletActionContext.getCurrentRequest();
			// String imgFilePath =
			// request.getServletContext().getRealPath("/WEB-INF/classes/file/temp.png");
			String imgFilePath = "D:/test.png";// 新生成的图片
	...
	public static void TextToFile(String strBuffer) {
		try {
			// 创建文件对象
			File fileText = new File("c:\\1.txt");
			// 向文件写入对象写入信息
